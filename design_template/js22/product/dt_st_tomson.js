...
$(document).ready(function () {
    ...
    initCount(50, "tomson_count", 1);

    monoYn = $("#dt_st_tomson_mono_yn").val();
    prdtDvs = $("#prdt_dvs").val();
    sortcode = $("#dt_st_tomson_cate_sortcode").val();
    cateName = $("#dt_st_tomson_cate_sortcode").find("option:selected").text();
    amtUnit = $("#dt_st_tomson_amt").attr("amt_unit");
    var im = $("#im").val();

    calcManuPosNum.defWid = parseFloat($("#dt_st_tomson_size").attr("def_cut_wid"));
    calcManuPosNum.defVert = parseFloat($("#dt_st_tomson_size").attr("def_cut_vert"));

    var order_cut_wid = $("#order_cut_wid").val();
    var order_cut_vert = $("#order_cut_vert").val();
    var order_stan_name = $("#order_stan_nmae").val();

    $("#dt_st_tomson_size_name").val(order_stan_name);
    if(order_cut_wid != "" && order_cut_vert != "") {
        $("#dt_st_tomson_cut_wid_size").val(order_cut_wid);
        $("#dt_st_tomson_cut_vert_size").val(order_cut_vert);
...
var changeData = function () {
    ...
    $("input[name='chk_opt[]']").each(function (i) {
        if ($(this).val() === "당일판") {
            var amt = $("#dt_st_tomson_amt").val();
            if(amt > 490){
                $("#dt_st_tomson_amt").val(490).prop("selected", "true");
                alert("당일판은 500장 이상 주문이 불가합니다. 이하로 주문해주세요");
            }
        }
    ...
    var data = {
        "dvs": prdtDvs,
        "mono_yn": monoYn,
        "cate_sortcode": $("#dt_st_tomson_cate_sortcode").val(),
        "amt": $("#dt_st_tomson_amt").val(),
        "count": $("#count").val(),
        "size": $("#dt_st_tomson_size").val()
    };

    data.paper = $("#dt_st_tomson_paper").val();
    data.bef_tmpt = $("#dt_st_tomson_print_tmpt").val();
    data.bef_add_print_mpcode = '';
    data.aft_print_mpcode = '';
    data.aft_add_print_mpcode = '';
    data.print_purp = $("#dt_st_tomson_print_purp").val();
    data.page_info = "2";
    data.cut_wid_size = $("#dt_st_tomson_cut_wid_size").val();
    data.tomson_count = $("#tomson_count").val();
    data.cut_vert_size = $("#dt_st_tomson_cut_vert_size").val();
    data.minimum_amt = $("#minimum_amt").val();
    data.size_name = $("#dt_st_tomson_size").find("option:selected").text();
    if ($("#size_dvs").val() === "manu") {
        data.def_val =
            $("#dt_st_tomson_size").attr("def_val");
        data.def_stan_name =
            $("#dt_st_tomson_size > option[value='" + data.def_val + "']").html();
        data.manu_pos_num =
            $("#manu_pos_num").val();
        data.amt_unit =
            $("#dt_st_tomson_amt").attr("amt_unit");
    }

    var i = 0;
    $("input[type='checkbox'][name='dt_st_tomson_chk_after[]']").each(function () {
        if ($(this).prop("checked") === false) {
            return true;
        }

        var aft = $(this).attr("aft");
        var aft_depth = $("#dt_st_tomson_" + aft).val() == undefined ? "" : $("#dt_st_tomson_" + aft).val();
        var aft_depth1 = $("#dt_st_tomson_" + aft + "_1").val() == undefined ? "" : $("#dt_st_tomson_" + aft + "_1").val();
        var aft_depth2 = $("#dt_st_tomson_" + aft + "_2").val() == undefined ? "" : $("#dt_st_tomson_" + aft + "_2").val();
        var aft_depth1_val = $("#dt_st_tomson_" + aft + "_val").val() == undefined ? "" : $("#dt_st_tomson_" + aft + "_val").val();
        var aft_depth1_vh = $("#dt_st_tomson_" + aft + "_vh").val() == undefined ? "" : $("#dt_st_tomson_" + aft + "_vh").val();
        var aft_depth1_cnt = $("#dt_st_tomson_" + aft + "_cnt").val() == undefined ? "" : $("#dt_st_tomson_" + aft + "_cnt").val();
        var aft_depth1_dvs = $("#dt_st_tomson_" + aft + "_dvs_1").val() == undefined ? "" : $("#dt_st_tomson_" + aft + "_dvs_1").val();
        var aft_depth2_dvs = $("#dt_st_tomson_" + aft + "_dvs_2").val() == undefined ? "" : $("#dt_st_tomson_" + aft + "_dvs_2").val();
        var aft_depth1_wid = $("#dt_st_tomson_" + aft + "_wid_1").val() == undefined ? "" : $("#dt_st_tomson_" + aft + "_wid_1").val();
        var aft_depth1_vert = $("#dt_st_tomson_" + aft + "_vert_1").val() == undefined ? "" : $("#dt_st_tomson_" + aft + "_vert_1").val();
        var aft_depth2_wid = $("#dt_st_tomson_" + aft + "_wid_2").val() == undefined ? "" : $("#dt_st_tomson_" + aft + "_wid_2").val();
        var aft_depth2_vert = $("#dt_st_tomson_" + aft + "_vert_2").val() == undefined ? "" : $("#dt_st_tomson_" + aft + "_vert_2").val();
        ...
        if (aft == "foil") {
            data.aft_depth += $("#dt_st_tomson_" + aft + "_depth").val();
            data.after_name += $("#dt_st_tomson_" + aft + "_name").val();
        } else {
            data.after_name += aft;
...
var loadPrdtPrice = {
    ...
    "exec": function () {
        ...
        var callback = function (result) {
            ...
            $("input[name='dt_st_tomson_chk_after[]']:checked").each(function () {
                var $obj = $(this);
                var aft = $obj.attr('id').replace("dt_st_tomson_", "");
                if (aft == "foil") {
                    var totalAfterPrice = 0;
                    afterPrice = loadPrdtPrice.price["foil1"];
                    if (afterPrice) {
                        setAfterPrice("dt_st_tomson", "foil1", afterPrice);
                        totalAfterPrice += parseInt(afterPrice);
                    }
                    afterPrice = loadPrdtPrice.price["foil2"];
                    if (afterPrice) {
                        setAfterPrice("dt_st_tomson", "foil2", afterPrice);
                        totalAfterPrice += parseInt(afterPrice);
                    }
                    afterPrice = loadPrdtPrice.price["foil3"];
                    if (afterPrice) {
                        setAfterPrice("dt_st_tomson", "foil3", afterPrice);
                        totalAfterPrice += parseInt(afterPrice);
                    }
                    setAfterPrice("dt_st_tomson", "foil", totalAfterPrice);
                } else {
                    afterPrice = loadPrdtPrice.price[aft];
                    setAfterPrice("dt_st_tomson", aft, afterPrice);
                }
            });
...
var calcPrice = function (flag) {
    ...
    var posNum = 1;
    if ($("#size_dvs").val() === "manu") {
        posNum = parseFloat($("#dt_st_tomson_manu_pos_num").val());
    }
    // 건수
    ...
    gradeSale /= 100.0;
    // 회원 할인율
    var memberSale = parseFloat($("#dt_st_tomson_member_sale_rate").val());
    memberSale /= 100.0;
    // 옵션비 총합
...
var changeSizeDvs = function (val) {
    ...
    if (val === "manu") {
        $("#dt_st_tomson_manu_pos_num").val('1');
        $("#cut_wid_size").val($("#dt_st_tomson_size").attr("def_cut_wid"));
        $("#cut_vert_size").val($("#dt_st_tomson_size").attr("def_cut_vert"));
        $('#chgbutton').text("규격선택");
    }else if(val === "stan"){
        $('#chgbutton').text("직접입력");
    }
...
var changeDigitalCuttingSize = function() {
    console.log("changeDigitalCuttingSize");
    var work_wid_size = parseInt($("#work_wid_size").val());
    var work_vert_size = parseInt($("#work_vert_size").val());
    var cutting_count = calcProductCount(454,310, work_wid_size, work_vert_size);

    $("#minimum_amt").val(cutting_count);

    var now_amt = $("#dt_st_tomson_amt").val();
    var isExist = false;
    $("#dt_st_tomson_amt").html("");
    var dt_amt_html = "";
    ...
    $("#dt_st_tomson_amt").html(dt_amt_html);

    if(!isExist) $('#dt_st_tomson_amt option:eq(0)').prop('selected', true);
    rangeBarText();
    changeData();
}
...
var setSubmitParam = function () {
    ...
    var paperName = $("#dt_st_tomson_paper").find("option:selected").text();
    var sizeName = $("#dt_st_tomson_size").find("option:selected").text();
    var tmptName = $("#dt_st_tomson_print_tmpt").find("option:selected").text();
    ...
    $frm.find("input[name='dt_st_tomson_cate_name']").val(cateName);
    $frm.find("input[name='dt_st_tomson_amt_unit']").val(amtUnit);
    $frm.find("input[name='dt_st_tomson_paper_name']").val(paperName);
    $frm.find("input[name='dt_st_tomson_bef_tmpt_name']").val(tmptName);
    $frm.find("input[name='dt_st_tomson_size_name']").val(sizeName);

    $frm.find("input[name='dt_st_tomson_sell_price']").val(sellPrice);
    $frm.find("input[name='dt_st_tomson_sale_price']").val(salePrice);
    $frm.find("input[name='dt_st_tomson_after_price']").val(afterPrice);
