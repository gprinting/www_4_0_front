...
var setSizeWarning = function () {
    ...
    if (sortcode === "004001002") {
        if (cutW < 50 || cutW > 90) {
            alert("가로 사이즈 직접입력 시 최소 50mm ~ 최대 90mm 내에서 입력해 주세요. ");
            $(prefix + "cut_wid_size").val("50");
            return false;
        }
        if (cutH < 50 || cutH > 90) {
            alert("세로 사이즈 직접입력 시 최소 50mm ~ 최대 90mm 내에서 입력해 주세요. ");
            $(prefix + "cut_vert_size").val("50");
            return false;
