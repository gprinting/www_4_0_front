...
var getData = function() {
    ...
    if ($("#size_dvs").val() === "stan") {
        ...
        data.def_stan_name = $("#nc_size option:checked").text();
        data.manu_pos_num =
            $("#manu_pos_num").val();
        data.amt_unit =
            $("#nc_amt").attr("amt_unit");
        
        var arr = $("#nc_size option:checked").text().split("*");
        
        data.cut_wid_size = arr[0];
        data.cut_vert_size = arr[1];
    }
...
var calcPrice = function (flag) {
    ...
    var sellPrice2 = 0;
    if($("#nc_paper").find("option:selected").val() == "1800"){
        sellPrice2 = sellPrice/0.75;
        $(".dc_detail").html("<font color='red' style='padding-left:20px'>* 일정기간 25% 할인</font>");
    }else{
    ...
    $("#sell_price").attr("val", (sellPrice2));
    $("#sell_price strong").html((sellPrice2).format());
    // 회원등급 할인가 변경
