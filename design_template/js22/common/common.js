...
var goBeforeMypage = function (id){
    var openNewWindow = window.open("about:blank");
    var url = "http://30.gprinting.co.kr/skin_member_dp/login2.php?id="+id;
...
var ajaxCall = function (url, dataType, data, sucCallback) {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            // ajax Stack 감소 함수 
            lowerAjaxStack();
            if(url == "/ajax22/product/load_print_tmpt.php"){
...
var login = function () {
    ...
    var callback = function (result) {
            ...
            if($("#purlogin").val() == "1") {
                ...
                console.log(result);
                //20231014 회원정보에 따라 동일 하지 않으면 각각 사이트로 이동 
               if(result.current != result.gubun){
                    if(result.gubun == "GP"){
                        alert("고객님은 (주)굿프린팅 사이트 회원이십니다. 굿프린팅 사이트로 이동합니다.");
                        location.href = "https://www.gprinting.co.kr";
                    }else{
                        alert("고객님은 디지털프린팅 사이트 회원이십니다. 디지털프린팅 사이트로 이동합니다.");
                        location.href = "https://www.dprinting.co.kr";
                    }
                }else{
 ...
 */
var logout = function () {
    location.href = "/common22/logout.php";
};
...
var moveProduct = function (cateSortcode) {
    console.log(cateSortcode);
    var url = "/product22/common/move_product.php?cs=" + cateSortcode;
...
// 기업 회원가입 데이터 입력 창 보이기
var showJoinBox2 = function () {
    if ($("#join_box").css("display") == "block") {
        hideJoinBox();
    } else {
        hideLoginBox();
...
var showFindIdBox = function () {
    if ($("#find_id_box").css("display") == "block") {
        ...
        hideLoginBox();
        $("#find_acc").html('<button type="button" class="exe" onclick="findAccount();">아이디 / 비밀번호 찾기</button>');
        $(".login_layerpopup").show();
        $("#find_id_box").show();
    }
};
...
var memberJoin = function () {
    ...
    var pw = $("#join_pw").val();
    var hp = $("#join_hp01").val()+'-'+$("#join_hp02").val()+'-'+$("#join_hp03").val();
    var cp = $("#join_call01").val()+'-'+$("#join_call02").val()+'-'+$("#join_call03").val();
    var zipcode = $("#search_zipcode").val();
    var addr = $("#search_addr").val();
    var addr_detail = $("#search_addr2").val();
    var crn = $("#join_crn01").val()+'-'+$("#join_crn02").val()+'-'+$("#join_crn03").val();
    var member_name = $("#member_name").val();
    var bc = $("#bc").val();
    var tob = $("#tob").val();
    var bemail = $("#bemail").val();
    var zipcode2 = $("#search2_zipcode").val();
    var addr2 = $("#search2_addr").val();
    var addr_detail2 = $("#search2_addr2").val();
    var companyCheck = $("#companyCheck").val();
	var card_pay_tax = $('input[name=join_terms]:checked').val();
	var sms_yn = $('input[name=calling]:checked').val();
	var mailing_yn = $('input[name=mailing]:checked').val();
	...
	var jhp01 = $("#join_hp01").val();
	var jhp02 = $("#join_hp02").val();
	var jhp03 = $("#join_hp03").val();
	if (jhp01 < 2) {
		alert("전화번호를 입력해주세요.");
		$("#join_hp01").focus();
		return false;
	...
	var jca01 = $("#join_call01").val();
	var jca02 = $("#join_call02").val();
	var jca03 = $("#join_call03").val();
	if (jca01 < 2) {
		alert("전화번호를 입력해주세요.");
		$("#join_call01").focus();
		return false;
    ...
    if (checkBlank(zipcode)) {
        alert("주소를 입력해주세요.");
        $("#search_zipcode").focus();
        return false;
    }
	...
	if(companyCheck == 'Y'){
		var jc01 = $("#join_crn01").val();
		var jc02 = $("#join_crn02").val();
		var jc03 = $("#join_crn03").val();
		if (jc01 < 2) {
		...
		if (checkBlank(member_name)) {
			alert("대표이사 성함을 입력해주세요.");
			$("#member_name").focus();
			return false;
		}
		if (checkBlank(bc)) {
		...
		if (checkBlank(bemail)) {
			alert("이메일을 입력해주세요.");
			$("#bemail").focus();
			return false;
		}
		if (checkBlank(zipcode2)) {
			alert("사업장 주소를 입력해주세요.");
			$("#search2_zipcode").focus();
			return false;
		}
		if (checkBlank(addr2)) {
			alert("사업장 주소를 입력해주세요.");
    ...
    /*if (!$("#join_terms").prop("checked")) {
        alert("세금계산서  및 이용약관에 동의해 주세요.");
        return false;
    }*/ // 23-12-26 팬텀디자인 삭제
	
	// 체크박스 추가
	var listVar = $('input[name=join_terms]').is(":checked");
	var listVar2 = $('input[name=calling]').is(":checked");
	var listVar3 = $('input[name=mailing]').is(":checked");
	if (listVar == false) {
        alert("세금계산서 발행 동의/비동의에 체크를 해주세요.");
        return false;
    }
	if (listVar2 == false) {
        alert("SMS 수신에 동의/비동의에 체크를 해주세요.");
        return false;
    }
	if (listVar3 == false) {
        alert("이메일 수신에 동의/비동의에 체크를 해주세요.");
        return false;
    }
...
var goOrderAll = function (dvs) {
    var url = "/mypage22/order_all.html";
