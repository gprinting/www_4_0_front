...
var orderNextFunc = function () {
    ...
    var callback = function (result) {
        ...
        var need_file_upload = $("#need_file_upload").val()
        if(need_file_upload == "N") {
            fileNextFunc();return;
        }

        var cateCode = $("#vo_cate_sortcode").val();

        if(cateCode == "014002001"){
            alert("주문하신 내역은 : "+$(".order_detail").text().trim()+ " 입니다.");
        }
        ...
        var regExp = /[\{\}\[\]\/?.,;:|\)*~`!^\-+<>@\#$%&\\\=\(\'\"]/gi;
        var title2 = $("#file_name").val();
        saebalArr = title2.split('.');

       if(regExp.test(saebalArr[0])){
            
            alert("파일명의 특수문자를 제거해주세요 ");
            return false;
        ...
        if (checkBlank($("#title").val())) {
            ...
            alert("인쇄물 제목을 넣지 않았습니다. 인쇄물제목을 넣어주세요.");
            $('#title').focus();
            return false;
...
var fileNextFunc = function () {
    ...
    if (checkBlank(file_seqno) && order_common_seqno == '' && need_file_upload != "N") {
    ...
    $("#frm").attr("action", "/order22/add_cart.html");
    $("#frm").submit();
};
...
var loadPaperName = function (dvs, sort, callback) {
    var url = "/ajax22/product/load_paper_name.php";
    var data = {
...
var showDeliveryPop = function () {
    var url = "/ajax22/product/load_delivery_pop.php?";
    url += "cs=" + sortcode;
...
var loadDlvrPriceExpect = function (flag, cb = null) {
    ...
    var callback = function (result) {
        ...
        if($(prefix + "cate_sortcode").val() != "014005001"){
